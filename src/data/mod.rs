...
pub fn load_mapdata(json_source: &str) -> Result<MapData, String> {
    ...
    let areas: HashMap<String, String> = json["areas"].entries().map(|(k, v)| (k.into(), v.as_str().unwrap().into())).collect();

    let rooms: HashMap<String, Room> = json["rooms"].entries().map(|(k, v)| {
        Ok((k.into(), Room::try_from(v).map_err(|s| format!("{k}: {s}"))?))
